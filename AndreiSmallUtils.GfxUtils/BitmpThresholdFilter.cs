...
namespace AndreiSmallUtils.GfxUtils
{
     ...
     * 
     */
    public class BitmpThresholdFilter
    {
        // function is used to compute the q values in the equation
        private static float Px(int init, int end, IReadOnlyList<int> hist)
        {
            int sum = 0;
            int i;
            for (i = init; i <= end; i++)
        ...
        private static float Mx(int init, int end, IReadOnlyList<int> hist)
        {
            int sum = 0;
            int i;
            for (i = init; i <= end; i++)
        ...
        private static int FindMax(IReadOnlyList<float> vec, int n)
        {
            float maxVec = 0;
            int idx=0;
            int i;
        ...
        public int GetThreshold(Bitmap bmp)
        {
            ...
            byte t = (byte)FindMax(vet, 256);

            return t;
        }

        // simple routine to convert to gray scale
        public void Convert2GrayScale(Bitmap bmp)
        {
            var bmData = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height),
